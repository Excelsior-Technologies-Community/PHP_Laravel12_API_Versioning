...
@section('content')
<div x-data="editProduct('{{ $id }}')" x-init="init()">
    <div class="max-w-3xl mx-auto">
        <!-- Page Header -->
        <div class="mb-6">
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Edit Product</h1>
            <p class="text-gray-500 dark:text-gray-400 mt-1">Update product via V2 API (ID: <span x-text="productId"></span>)</p>
        </div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
                <form @submit.prevent="submit">
                    ...
                    <div class="mt-6 flex justify-end space-x-3 pt-6 border-t border-gray-200 dark:border-gray-700">
                        ...
                        <button type="submit"
                                :disabled="submitting"
                                class="px-6 py-2 bg-yellow-600 hover:bg-yellow-700 text-white rounded-lg font-medium shadow-sm transition-colors disabled:opacity-50">
                            <template x-if="!submitting">Update Product</template>
...
function editProduct(productId) {
    return {
        ...
        async fetchProduct() {
            this.loading = true;
            try {
                const res = await fetch(`/api/v2/products/${this.productId}`, {
